...
var capacity = 0.0;
var flex_angle_v_array = [0,0,0];
var flex_angle_vr_array = [0,0,0];
var flex_angle_r_array = [0,0,0];
var onground_flag = 0;
var drop_flag = 0;

var BambiBucket = func {
			

	var payload = getprop("sim/model/BambiBucket/enabled");
	...
	var alt_agl = altitude * 0.3048 + getprop("/sim/model/BambiBucket/offset");
	var n_segments = 3;
	var segment_length = getprop("/sim/model/BambiBucket/factor");
	...
	setprop("/sim/model/BambiBucket/buckethitsground", onground_flag);
	...
	if (sniffer == 1) 
		...
		if (drop_flag > 1 and !onground_flag)
			...
			{
				#setprop("sim/model/BambiBucket/position-norm", 0);
				#sniffer = 0;
				drop_flag = 0;
			}

	var flex_force = getprop("/sim/model/BambiBucket/flex-force");
	var damping = getprop("/sim/model/BambiBucket/damping");
	var stiffness = getprop("/sim/model/BambiBucket/stiffness");
	var sum_angle = 0.0;
	var sum_roll = 0.0;
	var dt = getprop("/sim/time/delta-sec");
	var bend_force = getprop("/sim/model/BambiBucket/bendforce");
	var angle_correction = getprop("/sim/model/BambiBucket/correction");
	...
	if (onground_flag == 0)
	   ...
	   var current_angle = getprop("/sim/model/BambiBucket/pitch1");
	   ...
	   setprop("/sim/model/BambiBucket/pitch1", current_angle + dt * ang_speed);
	   
	   
	   var current_roll = getprop("/sim/model/BambiBucket/roll1");
	   ...
	   setprop("/sim/model/BambiBucket/roll1", current_roll + dt * roll_speed);
	   ...
	   #setprop("/sim/model/BambiBucket/roll2",  kink) ;
	   ...
	   setprop("/sim/model/BambiBucket/pitch1", ref_ang1);
	   setprop("/sim/model/BambiBucket/roll1", ref_ang2);
	...
	for (var i = 1; i< n_segments; i=i+1)
		...
		if (onground_flag == 0)
		  {
		  current_angle = getprop("/sim/model/BambiBucket/pitch"~(i+1));
		  ang_error = angle - current_angle;
		  
		current_roll = getprop("/sim/model/BambiBucket/roll"~(i+1));
		  ...
		  setprop("/sim/model/BambiBucket/pitch"~(i+1), current_angle + dt * ang_speed);
		  ...
		  setprop("/sim/model/BambiBucket/roll"~(i+1), current_roll + dt * roll_speed);

		  }
		else
		  {
		  setprop("/sim/model/BambiBucket/pitch"~(i+1), angle + angle_correction);
		  setprop("/sim/model/BambiBucket/roll"~(i+1), roll + angle_correction);
