...
var Tank = {
	...
	consumel: func(amount) { # US gal (neg. values for feeding)
		var level = me.level();
		if (amount > level)
			amount = level;
		level -= amount;
	...
	consumer: func(amount) { # US gal (neg. values for feeding)
		var level = me.level();
		if (amount > level)
			amount = level;
		level -= amount;
...
var fuel = {
	...
	update: func(dt) {
		...
		var etpa = props.globals.getNode("/systems/electrical/outputs/transfer-pump-aft").getValue() or 0;
		var etpf = props.globals.getNode("/systems/electrical/outputs/transfer-pump-forward").getValue() or 0;
		...
		if (free1 > 0) {
			var trans_flow1 = (trans1 + trans2) * me.pump_capacity;
			me.rsupply.consumer(-me.main.consumer(min(trans_flow1 * dt, free1)));
		}
		if (free2 > 0) {
			var trans_flow2 = (trans1 + trans2) * me.pump_capacity;
			me.lsupply.consumel(-me.main.consumel(min(trans_flow2 * dt, free2)));
		}
	...
	consumer: func(amount) {
		var PWR1 = getprop("/controls/engines/engine[0]/power");
		#var PWR2 = getprop("/controls/engines/engine[1]/power");
		return me.freeze ? 0 : me.rsupply.consumer(amount * PWR1);
	},
	
	consumel: func(amount) {
		#var PWR1 = getprop("/controls/engines/engine[0]/power");
		var PWR2 = getprop("/controls/engines/engine[1]/power");
		return me.freeze ? 0 : me.lsupply.consumel(amount * PWR2) ;
	},
...
var Engine = {
	...
	update: func(dt, trim = 0) {
		...
		if ((getprop("engines/engine[0]/running", "true")) or (getprop("engines/engine[1]/running", "true"))){
		...
		if (getprop("engines/engine[0]/running", "false")){
		...
		if (getprop("engines/engine[1]/running", "false")){
		...
		setprop("engines/engine[1]/running", "false");
		me.timer.stop();
		}else {
		setprop("engines/engine[1]/running", "true");
		me.fuelflow = power;
		...
		fuel.consumer(75 * KG2GAL * me.fuelflow * dt / 3600);
		fuel.consumel(75 * KG2GAL * me.fuelflow * dt / 3600);
